...
exports.obtenerAutoridadesByType = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(autoridades.filter((autoridad) => autoridad.tipo !== null));
  } catch (error) {
...
exports.traerAutoridadPorDocumentoOPorString = async (req, res) => {
  try {
    ...
    const useCase = {
      DOC: async () => {
        return autoridadService.traerPorDNI({
          numero_documento: value,
          tipo_autoridad_name: tipoAutoridadName,
        });
      },
      NOM: async () => {
        return autoridadService.traerAutoridadPorCoincidenciaEnNombre({
          cadena: value,
          tipo_autoridad_name: tipoAutoridadName,
        });
      },
      DEFAULT: () => {
        throw new Error(
          `'${type}' no es un valor aceptado para 'type'. Solo se admite: ['DOC','NOM'].`
        );
      },
    };

    const response =
      (useCase[type] && (await useCase[type]())) || useCase["DEFAULT"]();
    res.status(200).json(response);
  } catch (error) {
    res.status(400).send(responseService.crearBadResponse(error.message));
  }
